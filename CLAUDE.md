...
1. **AI 백엔드는 `anthropic/claude-sonnet-4-6` 단일 모델만 사용한다.** 모델 선택 분기, 변수화, 다른 모델 추가는 금지.
2. **WebUI(브라우저 Control UI) 기능을 포함한다.** `controlUi` 설정과 Gateway Token 안내, 브라우저 접속 흐름을 `openclaw.json` 및 `run.sh`에 유지한다.
3. **현재는 Telegram 단독 운영, 추후 Slack 추가 예정이다.** `openclaw.json`의 `channels`에는 현재 telegram만 활성화한다.
...
- `gateway.controlUi`는 gcube 리버스 프록시 대응을 위해 포함한다.
- `channels`에는 현재 `telegram`만 활성화한다 (추후 Slack 추가 예정).
...
- **`setup.sh` 수정 시**: API 키 검증은 `ANTHROPIC_API_KEY` 단독으로만 한다.
- **`run.sh` 수정 시**: 출력 메시지에 Gateway Token 및 WebUI 접속 안내(gcube URL 접속)를 포함한다.
- **`README.md` 수정 시**: Telegram 봇 이름은 `@openclaw_claude_da_bot`이다.
- **설정 JSON 수정 시**: 모델 ID는 반드시 `"anthropic/claude-sonnet-4-6"`이다.
