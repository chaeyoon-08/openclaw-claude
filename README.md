...
```mermaid
flowchart TD
    A[gcube 워크로드 배포] --> B[컨테이너 터미널 접속]
    B --> C[git clone]
    C --> D[.env 작성]
    D --> E[bash setup.sh]
    E --> F[bash run.sh]
    F --> G[gcube URL 접속]
    G --> H[Gateway Token 입력 후 Connect]
    H --> I{pairing required?}
    I -->|Yes| J[openclaw devices approve]
    J --> K[Health OK ✅]
    I -->|No| K
    K --> L[Telegram 봇 메시지 전송]
    L --> M[pairing 코드 수신]
    M --> N[openclaw pairing approve telegram 코드]
    N --> O[Telegram 봇 정상 작동 ✅]
```
