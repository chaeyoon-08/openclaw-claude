...
```mermaid
flowchart TD
    A[Gemini API 키 발급\nhttps://aistudio.google.com/apikey] --> B
    B[Telegram BotFather에서 봇 생성 \n 봇 토큰 저장] --> C
    C[gcube 워크로드 배포 \n 포트 8080] --> D
    D[컨테이너 터미널 접속] --> E
    E[git clone\ncd openclaw-webui] --> F
    F[cp .env.example .env \n nano .env에 API 키 입력] --> G
    G[bash setup.sh] --> H
    H[bash run.sh \n → 터미널에 Gateway Token 출력] --> I
    I[gcube 서비스 URL 브라우저 접속] --> J
    J[Overview 페이지 \n Gateway Token 필드에 토큰 붙여넣기 \n → Connect 클릭] --> K
    K{pairing required \n 메시지 뜨는가?} -->|Yes| L
    K -->|No| M
    L[터미널에서 승인 \n openclaw devices list \n openclaw devices approve requestId] --> M
    M[Health OK 확인 ✅] --> N
    N[Telegram에서 생성한 봇 검색 \n 아무 메시지 전송] --> O
    O[봇에서 pairing 코드 수신 \n 명령어 복사] --> P
    P[터미널에서 입력 \n openclaw pairing approve telegram 코드] --> Q
    Q[Telegram 봇 정상 작동 ✅ \n 대화 시작]
```
